...
const WaterDetailsSummaryPage: FC<IProps> = ({ navigation, route }) => {
  ...
  const navigateToEdit = () => navigation.navigate('WaterDetailsScreen', { id, surveyId, waterId: waterMeasurement?.id });
  ...
  useEffect(() => {
    const getMeasurement = async () => {
      const res = await getWaterMeasurements(id);
    ...
    if (waterMeasurement) {
      if (
        measurementStatusChecker(
          ...
          waterMeasurement?.description,
          waterMeasurement?.river_bed,
          waterMeasurement?.river_wall,
          waterMeasurement?.river_wall_color,
          waterMeasurement?.river_width_enough,
          waterMeasurement?.river_width_length,
          waterMeasurement?.water_discharge_id,
          waterMeasurement?.water_discharge_slug,
          waterMeasurement?.water_level_id,
          waterMeasurement?.water_level_slug,
          waterMeasurement?.water_discharge_speed,
        )
      ) {
        setStatus('Finished');
      } else {
      ...
      case 1:
        return (
          <WaterResults
            ...
            additionalNotes={waterMeasurement?.description}
            waterLevelChecked={waterMeasurement?.water_level_name}
            riverDischarge={waterMeasurement?.water_discharge_name}
            riverWidth={waterMeasurement?.river_width_length}
            riverBed={waterMeasurement?.river_bed}
            riverWall={waterMeasurement?.river_wall}
            riverWallColor={waterMeasurement?.river_wall_color}
            riverFlow={waterMeasurement?.water_discharge_speed}
            wildLife={waterMeasurement?.wildlife}
            selectedRiverFlow={undefined}
            goBack={goBack}
            navigateToEdit={navigateToEdit}
            //@ts-ignore
            selectedRiverDischarge={undefined}
          />
        );
    }
  };
...
export default WaterDetailsSummaryPage;
